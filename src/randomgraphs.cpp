...
inline static void validateProbability(double p) {
    if (!(p >= 0 && p <= 1))
        throw std::invalid_argument("Argument (" + std::to_string(p) +
...
inline static void validateEdgeNumber(size_t n, size_t m) {
    if (m > n * (n - 1) * .5)
        throw std::invalid_argument("Argument (" + std::to_string(m) +
                                    ") is not a valid edge number"
                                    "Value must be in [0," +
                                    std::to_string(n * (n - 1) * .5) + "].");
}
...
static UndirectedGraph generateErdosRenyiRandomGraphWithRetries(size_t n,
                                                                size_t m) {
    ...
    size_t newEdgeIndex;
    bool newEdgeFound;
    auto uniformEdgeDistribution =
        std::uniform_int_distribution<size_t>(0, n * (n - 1) * .5 - 1);
    ...
    for (size_t i = 0; i < m; i++) {
        ...
        while (!newEdgeFound) {
            ...
            newEdgeFound =
                existingEdges.find(newEdgeIndex) == existingEdges.end();
        }
        existingEdges.insert(newEdgeIndex);
        auto edge = getUndirectedEdgeFromIndex(newEdgeIndex, n);
        randomGraph.addEdge(edge.first, edge.second, true);
    }
    return randomGraph;
}

// Slightly slower for small edge number
static UndirectedGraph generateErdosRenyiRandomGraphFisherYates(size_t n,
                                                                size_t m) {
    validateEdgeNumber(n, m);
    ...
    for (size_t i = 0; i < m; i++) {
        newEdgeIndex =
            std::uniform_int_distribution<size_t>(i, maxEdgeNumber)(rng);
        ...
        if (edgeReplacements.find(newEdgeIndex) == edgeReplacements.end()) {
            ...
            auto edge =
                getUndirectedEdgeFromIndex(edgeReplacements[newEdgeIndex], n);
            randomGraph.addEdge(edge.first, edge.second, true);
        }
...
UndirectedGraph generateSmallWorldRandomGraph(size_t n, size_t d, double p) {
    ...
    if (!(1 <= d && d <= floor(.5 * (n - 1))))
        throw invalid_argument(
            "Argument (" + std::to_string(d) +
            ") is not a valid degree parameter for the small-world model."
            " Value must be in [1," +
            std::to_string(int(floor(.5 * (n - 1)))) + "].");
    ...
    for (size_t i = m + 1; i <= n * d; i++) {
        r_prime =
            std::uniform_int_distribution<size_t>(i, n * (n - 1) * .5 - 1)(rng);

        if (edgeReplacements.find(r_prime) == edgeReplacements.end()) {
            auto edge = getUndirectedEdgeFromIndex(r_prime, n);
            randomGraph.addEdge(edge.first, edge.second, true);
        } else {
            auto edge =
                getUndirectedEdgeFromIndex(edgeReplacements[r_prime], n);
            randomGraph.addEdge(edge.first, edge.second, true);
        }
...
UndirectedGraph generateGraphWithDegreeDistributionStubMatching(
    const vector<size_t> &degreeDistribution) {
    ...
    while (stubIterator != stubs.end()) {
        ...
        stubIterator++;
        if (stubIterator == stubs.end())
            break;

        if (vertex1 != *stubIterator &&
            !randomGraph.hasEdge(vertex1,
                                 *stubIterator)) // no loops and multiedges
...
vector<Edge> getEdgeVectorOfGraph(const UndirectedGraph &graph) {
    ...
    for (VertexIndex &vertex1 : graph)
        for (VertexIndex vertex2 : graph.getOutNeighbours(vertex1))
            if (vertex1 < vertex2)
...
void shuffleGraphWithConfigurationModel(UndirectedGraph &graph,
                                        vector<Edge> &edges, size_t swaps) {
    if (swaps == 0)
        swaps = 2 * graph.getEdgeNumber();
    ...
    for (size_t i = 0; i < swaps; i++) {
        ...
        if (newEdge1.first == newEdge1.second ||
            newEdge2.first == newEdge2.second)
            continue;

        if (graph.hasEdge(newEdge1.first, newEdge1.second) ||
            graph.hasEdge(newEdge2.first, newEdge2.second))
            continue;

        graph.removeEdge(currentEdge1.first, currentEdge1.second);
        graph.removeEdge(currentEdge2.first, currentEdge2.second);
        graph.addEdge(newEdge1.first, newEdge1.second, true);
        graph.addEdge(newEdge2.first, newEdge2.second, true);
